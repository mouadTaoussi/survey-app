...
this.addEventListener('fetch',(e)=>{
	e.respondWith(
		caches.match(e.request).then((cach)=>{
...
caches.match(e.request).then((responseFromCach)=>{
			if (!responseFromCach){
				fetch(e.request).then((responseFromServers)=>{
					// console.log(responseFromServers)
					caches.open(dynamicAssets).then((cach)=>{
						cach.put(e.request.url,responseFromServers.clone());
					})
				...
				.catch((err)=>{
					...
					// caches.open(cachStaticVersion).then((cach)=>{
					// 	cach.match('./fallbackPage.html').then((fallbackPage)=>{
					// 		return fallbackPage;
					// 	})
					// })
				});
			}
			return responseFromCach || responseFromServers
		})